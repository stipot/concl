...
from llm_provider import LLMOptions, OpenAIProvider, OpenAICompatProvider, is_reasoning_model
...
def call_llm(
    ...
    try:
        ...
        try:
            ...
            # Третья попытка — без response_format, с жёсткой инструкцией уже внутри провайдера
...
def cmd_gen(
    ...
    provider: Optional[str] = typer.Option(None, help="LLM провайдер: openai | openai-compat. Если не указан — из настроек llm.provider."),
