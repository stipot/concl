# -*- coding: utf-8 -*-
"""Multi-Head Attention ntransfer4.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Of2rN5X3yjHvt4rbKSGoiElhgnB-ZNaj
...
class Autoencoder(nn.Module):
    def __init__(self, latent_dim=1024):
        ...
        # Multi-Head Attention на выходе энкодера
    ...
    def forward(self, x):
        ...
        # Преобразуем для MHA: [B,512,2,2] -> [B,4,512] -> [4,B,512]
        z_seq = z.permute(0, 2, 3, 1).view(B, 4, 512).permute(1, 0, 2)
        attn_out, _ = self.mha(z_seq, z_seq, z_seq)  # [4,B,512]
        attn_out = attn_out.permute(1, 0, 2).contiguous().view(B, -1)  # [B,2048]

        z_latent = self.fc_enc(attn_out)
    ...
    def encode(self, x, require_grad=True):
        if not require_grad:
            with torch.no_grad():
                ...
                B = z.size(0)
                z_seq = z.permute(0, 2, 3, 1).view(B, 4, 512).permute(1, 0, 2)
                attn_out, _ = self.mha(z_seq, z_seq, z_seq)
                attn_out = attn_out.permute(1,0,2).contiguous().view(B,-1)
                z_latent = self.fc_enc(attn_out)
        else:
            z = self.encoder(x)
            B = z.size(0)
...
class BinaryClassifier(nn.Module):
    def __init__(self, embedding_dim=512):
        ...
        # Обратите внимание: для этого нужно, чтобы 64*8 = 512.
        # Или можно оставить embed_dim=512 и num_heads=8, тогда head_dim=64 автоматом.
        # Тогда просто self.mha = nn.MultiheadAttention(embed_dim=512, num_heads=8)
    ...
    def forward(self, z):
        # z: [B, 512]
        B = z.size(0)
        # Разбиваем на 8 токенов по 64 признака: z_seq [B,8,64]
        z_seq = z.view(B, 8, 64)
        # Переставляем для MHA: [L=8, B, 64]
        z_seq = z_seq.permute(1, 0, 2)

        attn_out, _ = self.mha(z_seq, z_seq, z_seq) # [8, B, 64]
        attn_out = attn_out.permute(1, 0, 2).contiguous().view(B, -1) # обратно в [B,512]
...
def visualize_reconstruction(autoencoder, data_loader, device="cuda"):
    ...
    # Берём один батч из data_loader
    images, _ = next(iter(data_loader))
    images = images.to(device)

    # Прогоняем через автокодировщик
    with torch.no_grad():
    ...
    # Отображаем 8 первых изображений
    for i in range(8):
        # Исходное изображение
        ax = axes[0, i]
        img = images[i].permute(1, 2, 0).detach().numpy()
        # Если данные были нормализованы, возможно, стоит их денормализовать
        # Здесь просто показываем как есть
        ...
        # Реконструкция
        ax = axes[1, i]
        recon_img = recon[i].permute(1, 2, 0).detach().numpy()
...
# Пример вызова:
visualize_reconstruction(autoencoder, test_loader, device="cuda" if torch.cuda.is_available() else "cpu")
